...
describe('Detect Mozilla Firefox Android Browser 84 version', () => {
	...
	describe(`Should be ${EQUAL}`, () => {
		[...deepFlatFromObject(uaDB.Android.v10.Firefox.v84)].forEach((ua, i) => {
			test(`Case #${++i}: ${ua}`, () => {
				browserizr.setUA(ua);
				expect(
					browserizr.detect(isFirefoxAndroidVersion(EQUAL, 84))
				).toBeTruthy();
			});
		});
	});
	...
	describe(`Should not be ${EQUAL}`, () => {
		[
			...
			test(`Case #${++i}: ${ua}`, () => {
				browserizr.setUA(ua);
				expect(browserizr.detect(isFirefoxAndroidVersion(EQUAL, 84))).toBeFalsy();
			});
		});
	});
	...
	describe(`Should be ${LESS_THEN_OR_EQUAL}`, () => {
		[
			...
			test(`Case #${++i}: ${ua}`, () => {
				browserizr.setUA(ua);
				expect(
					browserizr.detect(isFirefoxAndroidVersion(LESS_THEN_OR_EQUAL, 84))
				).toBeTruthy();
			});
		});
	});
